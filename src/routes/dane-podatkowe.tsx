  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Dane podatkowe (DAC7) — vlnd" },
      {
        ...
        content: "Dlaczego platformy cyfrowe muszą raportować dane sprzedawców do organów podatkowych.",
      },
...
function ProgressBar({ pct, level }: { pct: number; level: "ok" | "warn" | "required" }) {
  const color = level === "required" ? "bg-destructive" : level === "warn" ? "bg-amber-500" : "bg-primary";
  return (
    ...
    if (status.level === "required") {
      return (
        <div className="flex items-start gap-3 rounded-xl border border-destructive/40 bg-destructive/10 p-4">
          ...
          <p className="text-sm">
            Przekroczono próg DAC7 ({DAC7_TX_REQUIRED} transakcji lub {DAC7_PLN_REQUIRED.toFixed(0)} zł w roku). Uzupełnienie
            danych poniżej jest wymagane, abyśmy mogli zaraportować Twoją sprzedaż zgodnie z przepisami.
          </p>
        </div>
      );
    }
    if (status.level === "warn") {
      return (
        <div className="flex items-start gap-3 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4">
          ...
          <p className="text-sm">Zbliżasz się do progu DAC7 — warto uzupełnić dane podatkowe już teraz.</p>
        </div>
      );
    }
    return null;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="container mx-auto px-4 py-10 flex-1 max-w-3xl">
        ...
        <p className="text-muted-foreground mt-2">
          Zgodnie z unijną dyrektywą DAC7 platformy internetowe, na których sprzedajesz towary lub usługi, mają obowiązek
          raportować dane sprzedawców do organów podatkowych po przekroczeniu progu 30 transakcji lub 2 000 EUR w roku
          kalendarzowym. Prosimy o uzupełnienie danych, aby móc kontynuować sprzedaż bez przerw.
        </p>

        <div className="mt-4 rounded-xl border border-border/40 bg-muted/30 p-4 flex items-start gap-3">
          <Info className="h-5 w-5 text-primary shrink-0 mt-0.5" />
          <p className="text-sm text-muted-foreground">
            Dane są wykorzystywane wyłącznie do obowiązkowego raportowania podatkowego i nie są udostępniane publicznie
            ani innym użytkownikom.
          </p>
        </div>
          ...
          <div className="mt-8 space-y-4 rounded-2xl border border-border/40 bg-gradient-surface p-5">
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-muted-foreground">{status.txCount} / {DAC7_TX_REQUIRED}</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-muted-foreground">{status.grossPln.toFixed(2)} zł / {DAC7_PLN_REQUIRED.toFixed(0)} zł</span>
              </div>
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input className="mt-1" value={addressLine} onChange={(e) => setAddressLine(e.target.value)} required />
            </div>
            <div>
            ...
            <div>
              ...
              <Input className="mt-1" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} required />
            </div>
            <div>
              <Label>Kraj</Label>
              <Input className="mt-1" value={country} onChange={(e) => setCountry(e.target.value)} required />
            </div>
          </div>

          <div>
            <Label>{sellerKind === "business" ? "NIP" : "PESEL lub NIP"}</Label>
            <Input className="mt-1" value={tin} onChange={(e) => setTin(e.target.value)} placeholder="tylko cyfry" required />
          </div>
          ...
          {sellerKind === "private" ? (
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input className="mt-1" type="date" value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} />
              </div>
              <div>
                <Label>Miejsce urodzenia</Label>
                <Input className="mt-1" value={birthPlace} onChange={(e) => setBirthPlace(e.target.value)} />
              </div>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input className="mt-1" value={businessRegNo} onChange={(e) => setBusinessRegNo(e.target.value)} required />
              </div>
            </div>
          )}

          <Button type="submit" disabled={mutation.isPending} className="bg-gradient-primary text-primary-foreground shadow-glow">
